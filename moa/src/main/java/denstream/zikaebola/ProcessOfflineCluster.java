...
public class ProcessOfflineCluster 
{
			...
			ArrayList<Integer> remained_points_idx_list)
	{
		...
		int remained_point_count = remained_points_idx_list.size();
		for (int iPoint=0; iPoint<remained_point_count; iPoint++)
		{	
			double temp_x = point_x_list[remained_points_idx_list.get(iPoint)];
			double temp_y = point_y_list[remained_points_idx_list.get(iPoint)];
			...
			if (overall_min_dist < den.epsilonOption.getValue()*den.epsilonOption.getValue()*4)
			{
				...
				nearest_micro_cluster.related_point_idxs.add(remained_points_idx_list.get(iPoint));
			}
		}
	}
}
