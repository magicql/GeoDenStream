...
public class WithDBSCAN extends AbstractClusterer {
	...
	ArrayList<Integer> initBufferPointIndex;
	...
	private ArrayList<MicroCluster> removalList = new ArrayList<MicroCluster>();
	...
	public void resetLearningImpl() {
		...
		initBufferPointIndex = new ArrayList<Integer>();
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (!initialized) {
			...
			initBufferPointIndex.add(((DenseInstance)inst).getInstanceId());
			...
			if (!merged && (o_micro_cluster.getClustering().size() != 0)) {
				...
				if (xCopy.getRadius(timestamp) <= epsilon) {
					...
					if (x.getWeight() > beta * mu) {
						...
						//! try reusing previous potential-cluster id based on location
			...
			if ((cur_time.getTime() - start_time_long) % time_step_milliseconds == 0)
			{
				...
				for (Cluster c : removalList) 
				{
					...
					//!record IDs of the removed potential-clusters
					if (temp_c.getId() > 0)
					{
